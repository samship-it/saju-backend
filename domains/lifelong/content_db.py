...
네 개의 테이블로 나뉜다(키 공간을 최소화하기 위함 — domains/lifelong/service.py
...
- domains_db: (일주, 현재 대운의 지배 십신군, 대운 순번) 조합당 1건 — 삶의 4대 영역
  ...
  고정 앵커로 서로 겹치지 않게 한다.
- stage_db: (일주, 대운 순번, 지배 십신군, 충형관계) 조합당 1건 — 메인 평생운세 화면에
  바로 보이는 '티저'(theme_line/keyword)만. 60×(5×8×충형카테고리) = 14,000건.
- stage_detail_db: stage_db 와 동일한 키 — '대운 상세 리포트' 전용(event_narrative/
  strategy/obstacle/turning_point). 클릭해야 보이는 전용 엔드포인트에서만 조회한다.
  scripts/generate_content_db.py --domain lifelong_stage 가 한 번의 생성 호출로
  6개 필드를 함께 만들어 stage_db/stage_detail_db 두 파일로 나눠 쓴다(생성 비용을
  늘리지 않기 위함).

네 파일 모두 배포에 포함되는 정적 산출물이므로, 프로세스 시작 후 최초 1회만 읽어
메모리에 둔다. DB 를 다시 생성했다면 프로세스를 재시작하거나 `reload()` 를 호출한다.

키 형식:
- base: "<일주>"                                    예) "甲子"
- domains: "<일주>_<dominant_group>_<순번>"          예) "甲子_재성_3"
- stage / stage_detail: "<일주>_<dominant_group>_<충형관계>_<순번>"
"""
import json
import os
import threading
...
STAGE_DETAIL_DB_PATH = os.environ.get(
    "LIFELONG_STAGE_DETAIL_DB_PATH", os.path.join(DATA_DIR, "lifelong_stage_detail_db.json")
)
...
_stage_detail_cache: Optional[Dict[str, Any]] = None
...
def load_stage_detail_db(force: bool = False) -> Dict[str, Any]:
    global _stage_detail_cache
    if _stage_detail_cache is not None and not force:
        return _stage_detail_cache
    with _lock:
        if _stage_detail_cache is not None and not force:
            return _stage_detail_cache
        _stage_detail_cache = _load(STAGE_DETAIL_DB_PATH)
        return _stage_detail_cache


def reload() -> None:
    """네 DB 파일을 다시 생성한 뒤 메모리 캐시를 갱신할 때 호출(테스트/재생성용)."""
    load_base_db(force=True)
    load_domains_db(force=True)
    load_stage_db(force=True)
    load_stage_detail_db(force=True)
...
def lookup_stage(ilju: str, dominant_group: str, branch_relation: str, step: int) -> Optional[Dict[str, Any]]:
    """티저 필드(theme_line/keyword)만. 상세는 lookup_stage_detail() 참고."""
    return load_stage_db().get(stage_key(ilju, dominant_group, branch_relation, step))


def lookup_stage_detail(ilju: str, dominant_group: str, branch_relation: str, step: int) -> Optional[Dict[str, Any]]:
    """대운 상세 리포트 필드(event_narrative/strategy/obstacle/turning_point)."""
    return load_stage_detail_db().get(stage_key(ilju, dominant_group, branch_relation, step))
...
def stats() -> Dict[str, Any]:
    return {
        ...
        "stage_detail_path": STAGE_DETAIL_DB_PATH, "stage_detail_count": len(load_stage_detail_db()),
        "stage_detail_expected": 14000,
    }
