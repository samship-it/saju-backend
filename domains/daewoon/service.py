...
- domain_pipeline(2026-09-21 신규 → 같은 날 v2 전면 재설계, domains/daewoon/pipeline.py):
  원국 분석→대운 분석→9영역(자아/성장·학업/전문성·직업/사회·재물·가족/부모·형제자매·
  친구/인간관계·애정/연애·결혼/배우자) 매핑→영향도(activation 0~5)+영역별 독립 valence/
  confidence/drivers/manifestation_types→고영역(3점 이상)만 구체 해석→자연어 합성의
  7단계 파이프라인 결과. domain_scores에는 activation>=3이고 이 나이의 생애단계(아동/
  청소년/청년/성인)가 그 영역의 허용 단계에 포함되는 항목만 담긴다 — 저영역·생애단계
  부적합 영역(예: 20세 미만의 결혼/배우자)은 "변화 없음" 문구조차 없이 완전히 제외된다
  (v1은 저영역을 고정 문구로 노출했으나 사용자가 전면 비노출로 재지정). 20세 미만은
  학업/자아/가족/친구, 20세 이상은 직업/재물/애정·결혼 영역에 +1 가중치가 자동으로
  붙는다. narrative는 "[10년의 풍경]"(decade_theme 아래) 섹션에 이어 붙이는 용도.
"""
